...
import { mockWorkers, mockWorkerStats } from '../mock/data';
...
const WorkerDetail = () => {
  ...
  const fetchWorkerDetail = async () => {
    ...
    try {
      ...
      const mockWorker = mockWorkers.find(w => w.id === Number(id));
      if (mockWorker) {
        setWorker(mockWorker as unknown as Worker);
        setGpus((mockWorker.gpu_devices || []) as unknown as GPU[]);
        const workerStats = mockWorkerStats[Number(id)];
