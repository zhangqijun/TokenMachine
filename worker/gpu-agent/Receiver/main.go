...
func init() {
	...
	config = Config{
		ReceiverPort:     9001,
		LogFile:          "/var/run/tokenmachine/receiver.log",
		WorkDir:          "/var/run/tokenmachine",
		MaxConcurrent:    10,
		DefaultVLLMImage: "vllm/vllm:latest",
	}
...
func startVLLM(taskID string, config map[string]interface{}) error {
	...
	vllmImage := getString(config, "vllm_image", "vllm/vllm:latest")
	...
	// 构建容器名称
	containerName := fmt.Sprintf("vllm-%s", taskID)
	...
	// 添加 GPU 参数
	if len(gpuIDs) > 0 {
		dockerArgs = append(dockerArgs, "--gpus", strings.Join(gpuIDs, ","))
	}
	...
	logf("Starting container: %s, command: docker %s", containerName, strings.Join(dockerArgs, " "))
	...
	logf("Container started successfully: %s", containerName)
	return nil
}
...
func main() {
	...
	logf("Starting GPU Agent Receiver")
